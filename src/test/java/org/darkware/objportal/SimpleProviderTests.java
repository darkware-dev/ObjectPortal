...
public class SimpleProviderTests
{
    ...
    {
        PortalContextToken token = provider.requestNewContext();
        provider.getPortalContext(token).place(Integer.class, 99);
    ...
    {
        PortalContextToken defToken = provider.getDefaultToken();

        PortalContextToken token = provider.requestNewContext();
        provider.getPortalContext(token).place(Integer.class, 99);

        assertThat(defToken).isNotEqualByComparingTo(token);
        assertThat(provider.getPortalContext(token)).isNotSameAs(provider.getPortalContext(defToken));
    ...
    public void context_changeDefault()
    {
        ...
        assertThat(provider.getPortalContext().take(Integer.class)).isEqualTo(42);

        provider.useDefaultToken(token);

        assertThat(provider.getPortalContext().take(Integer.class)).isEqualTo(99);
    }
